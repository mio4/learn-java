...
- 技术的产生总是有原因的，学习时需要考虑到前因后果。
  - 框架层面
    - 原生Servlet语法——>Spring框架的出现可以让工程师更加关注业务的实现，而不是语言规定。
    - 汇编语言——>C++/Java高级语言的出现，封装了操作系统的底层实现，让程序员更加关注业务逻辑，而不是底层原理。
    - MySQL关系型数据库——>Redis等非关系型数据库的出现，更加适应于分布式服务器，速度更快。
    - 单机服务器——>微服务架构，异地容灾etc，让服务更加稳定，团队开发效率更高。
  - 语法层面
    - 线程池的出现，缓解了创建和销毁线程的开销问题。
- 学会分析
  - 技术的优缺点
  - 技术的使用场景

-  技术之间存在耦合关系，学习时需要形成知识系统，系统型的知识不容易遗忘
  - 逻辑分明的笔记能够唤醒[WAKE  UP]记忆
  - 技术的本质是逻辑，逻辑是解决逻辑的最佳路径
...
【同步和异步：线程的调用是调用就得到结果还是立即返回，有结果的时候再通知】
...
##### 3. 为什么连接的时候是三次握手，关闭的时候却是四次握手？【为什么关闭连接需要四次挥手】
...
1. 对于某些知识，就算忘掉了细节，但是也要说出自己知道的部分
