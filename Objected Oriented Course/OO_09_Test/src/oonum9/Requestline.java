...
public class Requestline extends Thread{
    ...
    public Requestline(TaxiGUI gui, mapInfo map,Tray tray,guiInfo gi, Taxi[] taxiline,Putout putout,Scanner scan) {
    	/**
		 * @REQUIRES: (\exist  tray, scan);
		 * @MODIFIES: none;
		 * @EFFECTS: Initialize class Requestline; 
		 */
  	  this.gui = gui;
  	  this.map = map;
  	  this.tray = tray;
    ...
    public void scheduler(Point p1,Point p2) {
    	/**
		 * @REQUIRES: (\exist Point p1, p2);
		 * @MODIFIES: tray.reation;
		 * @EFFECTS:  get location from tray and choose car to serve;
		 */
    ...
    public void run() {
		 ...
		 * @REQUIRES: (\exist scan);
		 * @MODIFIES: none;
		 * @EFFECTS:  get cr from scan and schedule;
		 */
    	String str;
   	    String[] strarray;
   	    int[] term = new int[4];
		...
		while(true) {
			...
			for(same = 0; same<i;same++) {
				if(requestl[same][0]==term[0]&&requestl[same][1]==term[1]&&requestl[same][2]==term[2]&&requestl[same][3]==term[3]) judge1 = false;
			}
			if(judge1 == false) {
				judge1 = true;
