...
public class RequestDeal extends Thread{
	...
	private Taxi2[] taxis = new Taxi2[100];
	...
	public void run() {
		try {
			...
			if(str.equals("y")) {
				...
				GuiInfo.initmatrix();
				for(int i=0; i < 100;i++) { //!!!
					taxis[i] = new Taxi2(i,readFile.getMap(),taxiGUI,GuiInfo,flowMonitor);
	 ...
	 */
	public Taxi2[] getTaxi(){ //将RequestDeal类里出租车传送到Main函数里
