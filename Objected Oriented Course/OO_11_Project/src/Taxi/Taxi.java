...
public class Taxi extends Thread{
	...
	//VIP相关
	private boolean vip = false; //是否是vip出租车
	private ArrayList<VIPRequest> iter = new ArrayList<VIPRequest>();
	private VIPRequest tempVIPRequest;
	...
	/** @REQUIRES : None;
	 * @MODIFIES : this.state,this.credit,this.posx,this.posy;
	 * @EFFECTS : \request.equals(change the position of the taxi and deal the request by the passenger);
	 */
	public void dealOrder(int x,int y,String type){
		...
		int direction = 0;
		while(posx!=x || posy!=y){
			tempVIPRequest.addPath(posx,posy);
			if(vip) {
				//System.out.println("???");
				TestIterator.outToFileTaxi(time, new Point(), new Point(), new Point(posx, posy), 3,this.id);
			}
		...
		//vip
		if(vip && type.equals("Service")){
			iter.add(tempVIPRequest);
	...
	public void changeToSend(String str,int srcx, int srcy, int dstx, int dsty, long time) {
		...
		this.relaxCnt = 0; //重置
		if(vip){
			tempVIPRequest = new VIPRequest();
			tempVIPRequest.setReq(str,srcx,srcy,dstx,dsty,time);
			tempVIPRequest.setTaxiCor(posx,posy);
			iter.add(tempVIPRequest);
			TestIterator.outToFileTaxi(time,new Point(),new Point(),new Point(),-1,this.id);
			TestIterator.outToFileTaxi(time,new Point(),new Point(),new Point(),0,this.id);
			TestIterator.outToFileTaxi(time,new Point(srcx,srcy),new Point(),new Point(),1,this.id);
			TestIterator.outToFileTaxi(time,new Point(),new Point(dstx,dsty),new Point(),2,this.id);
		}
	}
