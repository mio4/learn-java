...
public class Window extends Thread{
	...
	/** @REQUIRES : taxis!=null && GuiInfo!=null && taxiGUI!=null;
	 * @MODIFIES : this.taxis,this.GuiInfo,this.taxiGUI;
	 * @EFFECTS : this.taxis == taxis;
	 * this.GuiInfo == GuiInfo;
	 * this.taxiGUI == taxiGUI;
	 */
	public Window(Taxi2[] taxis, guiInfo GuiInfo, TaxiGUI taxiGUI) {
	...
	/** @REQUIRES : None;
	 * @MODIFIES : this.str,this.srcx,this.scry,this.dstx,this.dsty,this.time;
	 * @Effects : None;
	 */
	...
	/** @REQUIRES : None;
	 * @MODIFIES : None;
	 * @Effects : \request.equals(检测合法的出租车,选择信用最高路径最短的出租车接单);
	 */
	@Override
	public void run() {
		try {
	...
	/** @REQUIRES : None;
	 * @MODIFIES : None;
	 * @Effects : \request.equals(在GUI中画格子);
	 */
	public void setGrid() {		//画个四格格子
		taxiGUI.RequestTaxi(new Point(this.srcx,this.srcy),new Point(this.dstx,this.dsty));
	}

	/** @REQUIRES : None;
	 * @MODIFIES : this.tUsed,this.taxis;
	 * @Effects : \request.equals(检查是否存在可以接单的出租车);
	 */
	public void Detect() {
		for(int i=0; i < 100; i++) {
			...
			if((tx>=srcx-2 && tx <= srcx+2) && (ty>=srcy-2 && ty <= srcy+2) && !tUsed[i] && tstate!=0 &&taxis[i].getTaxiState()==2) { //在格子内
				this.Ids.add(i);
				tUsed[i] = true;
